...
exports.getDashboardData = async (req, res) => {
    try{
        ...
        console.log(totalIncome, { userId, isValidObjectId: isValidObjectId(userId) });
        ...
        const lastIncomeTxns = (await Income.find({userId}).sort({date:-1}).limit(5)).map(
            (txn) => ({
                ...txn.toObject(),
                type: "income",
            })
        );
        const lastExpenseTxns = (await Expense.find({userId}).sort({date:-1}).limit(5)).map(
            (txn) => ({
                ...txn.toObject(),
                type: "expense",
            })
